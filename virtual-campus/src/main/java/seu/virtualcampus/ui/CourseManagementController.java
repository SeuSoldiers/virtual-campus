...
public class CourseManagementController {
    ...
    private void setupEventHandlers() {
        ...
        backButton.setOnAction(e -> DashboardController.handleBackDash("/seu/virtualcampus/ui/dashboard.fxml", backButton));
        // 为按钮添加提示文本
        ...
        executeApiRequest("http://" + MainApp.host + "/api/course/all",
                response -> {
                    try {
                        ...
                        List<Course> courseList = mapper.readValue(response, new TypeReference<>() {
                        });
                        Platform.runLater(() -> {
                ...
                error -> Platform.runLater(() -> {
                    ...
                    showAlert("加载失败", "无法加载课程列表", null, ERROR);
                })
        );
    }
    ...
    private void addCourse() {
        ...
        dialog.showAndWait().ifPresent(course -> executeApiPost("http://" + MainApp.host + "/api/course/add", course,
                () -> Platform.runLater(() -> {
                    loadCourses();
                    showAlert("成功", "课程添加成功", "操作成功", INFORMATION);
                }),
                error -> Platform.runLater(() ->
                        showAlert("添加失败", "课程添加失败: " + error, null, ERROR)
                )
        ));
    }
    ...
    private void editCourse(Course course) {
        ...
        dialog.showAndWait().ifPresent(updatedCourse -> {
            ...
            executeApiPut("http://" + MainApp.host + "/api/course/update/" + course.getCourseId(), updatedCourse,
                    () -> Platform.runLater(() -> {
                        loadCourses();
                        showAlert("成功", "课程更新成功", "操作成功", INFORMATION);
                    }),
                    error -> Platform.runLater(() ->
                            showAlert("更新失败", "课程更新失败: " + error, null, ERROR)
                    )
            );
        });
    }
    ...
    private void deleteCourse(Course course) {
        ...
        confirmAlert.showAndWait().ifPresent(response -> {
            if (response == confirmButton) {
                ...
                executeApiDelete("http://" + MainApp.host + "/api/course/delete/" + course.getCourseId(),
                        () -> Platform.runLater(() -> {
                            loadCourses();
                            showAlert("删除成功", "课程 '" + course.getCourseName() + "' 已成功删除", "操作成功", INFORMATION);
                        }),
                        error -> Platform.runLater(() -> {
                            loadingLabel.setText("删除失败");
                            showAlert("删除失败", "课程删除失败: " + error, null, ERROR);
                        })
                );
            }
        });
